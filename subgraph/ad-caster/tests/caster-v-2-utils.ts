...
export function createframeCastedEvent(
  frameId: string,
  cast: string
): frameCasted {
  let frameCastedEvent = changetype<frameCasted>(newMockEvent())

  frameCastedEvent.parameters = new Array()

  frameCastedEvent.parameters.push(
    new ethereum.EventParam("frameId", ethereum.Value.fromString(frameId))
  )
  frameCastedEvent.parameters.push(
    new ethereum.EventParam("cast", ethereum.Value.fromString(cast))
  )

  return frameCastedEvent
}

export function createframeCreatedEvent(
  frameId: string,
  publisher: Address,
  AdId: BigInt
): frameCreated {
  let frameCreatedEvent = changetype<frameCreated>(newMockEvent())

  frameCreatedEvent.parameters = new Array()

  frameCreatedEvent.parameters.push(
    new ethereum.EventParam("frameId", ethereum.Value.fromString(frameId))
  )
  frameCreatedEvent.parameters.push(
    new ethereum.EventParam("publisher", ethereum.Value.fromAddress(publisher))
